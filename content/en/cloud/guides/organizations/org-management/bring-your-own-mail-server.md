...
Because the test hangs up before naming a sender or a recipient, it cannot detect a server that accepts your credentials but refuses to send as your from address. Google Workspace's Gmail SMTP server is the common case: it accepts the login and then sends from the authenticated account instead. A passing test followed by mail arriving from the wrong address is that situation, and the fix is on the provider's side. See [Google Workspace](/cloud/guides/organizations/org-management/bring-your-own-mail-server/#google-workspace).
{{< /alert >}}
...
The constraint is the from address. Gmail sends as the authenticated account, or as an address that account has registered and confirmed as a **Send mail as** alias. A from address that is neither is replaced with the authenticated account's address, so your members see mail arrive from the account rather than from the address you configured. The connection test cannot catch this, because it hangs up before naming a sender. If you want to send as `no-reply@example.com` through this path, [add it as a Send mail as address](https://support.google.com/mail/answer/22370) on the authenticating account and complete Google's confirmation step first, or use Path A.
